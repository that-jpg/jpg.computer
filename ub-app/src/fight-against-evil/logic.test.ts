...
import { docCount, docCounts, docsByChapter, latestReview, type SolutionsManifest } from './logic'
...
const manifest: SolutionsManifest = {
  ...
  docs: [
    ...
    { id: '1.29', ch: 1, n: 29, file: '1.29.pdf', reviewed: '2026-09-01T14:44:27-03:00', verdict: 'wrong' },
...
describe('docsByChapter', () => {
  it('groups hrefs by chapter and item', () => {
    ...
    expect(docs.get(1)!.get(23)).toEqual({ href: '/fight-against-evil/solutions/1.23.pdf', verdict: 'correct' })
    expect(docs.get(1)!.get(29)).toEqual({ href: '/fight-against-evil/solutions/1.29.pdf', verdict: 'wrong' })
    expect(docs.get(2)!.get(4)!.href).toBe('/fight-against-evil/solutions/2.4.jpg')
...
describe('counts', () => {
  it('reports the number of docs and the newest review', () => {
    ...
    expect(docCounts(manifest)).toEqual({ correct: 2, wrong: 1 })
